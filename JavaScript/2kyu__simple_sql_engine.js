...
function SQLEngine(database) {
    const commands = ["select", "from", "where", "join", "on"];
    const splitRegex = /\s+(?=(?:[^']*'[^']*')*[^']*$)/;

    function nameObjectProperly(obj, name) {
    ...
    function parseQuery(queryStr) {
        queryStr = queryStr.replaceAll(",", "")
            .replaceAll("\'", "'")
            .replaceAll("<>", "!==");
        const query = { select: [], from: [], where: [], join: [], on: [] };
        let lastCommand = null;

        for (const word of queryStr.split(splitRegex)) {
            if (commands.includes(word.toLowerCase())) {
                lastCommand = word.toLowerCase();
            } else if (commands.includes(lastCommand)) {
                query[lastCommand].push(word);
            }
        }

        query.on = query.on.filter(v => v !== "=")
            .reduce((a, c, i, arr) => {
                if (i % 2 !== 0) {
                    a.push([arr[i - 1], c]);
                }
        ...
        query.where = query.where.reduce((acc, cur, i) => {
            if (i % 3 === 0) {
                acc.push(query.where.slice(i, i + 3)
                    .reduce((a, c, i, arr) => a + c + (i < arr.length - 1 ? " " : ""), ""));
            }
            return acc;
        }, []);
        return query;
    ...
    /*
    Here I use eval to filter the result so that the where condition holds. 
    This approach can be prone to SQL injection (literally), but I think 
    it's fine for the sake of this kata. I might refactor it at some point
    in the future..
    */
    function compare(value1, value2, operator) {
        const shouldStringify = typeof value1 === "string" || typeof value2 === "string";
        if (shouldStringify) {
            value1 = stringify(value1);
            value2 = stringify(value2);
        }
        return eval(`${value2}${operator === "=" ? '===' : operator}${value1}`);
    }

    function filter(table, query) {
        for (let condition of query.where) {
            const [field, operator, valueOrField] = condition.split(splitRegex);
            const [tableName, columnName] = field.split(".");
            const [tableName2, columnName2] = valueOrField.split(".");
            let value1 = columnName2 ? database[tableName2]
                .find((table) => table[columnName2])[columnName2] : valueOrField;

            value1 = String(!columnName2 && parseInt(value1) || value1);

            table = table.filter((row) => {
                let value2 = String(row[`${tableName}.${columnName}`]);
    ...
    function select(table, query) {
        return table.map(row => {
            let fields = {};
            for (let col in row) {
                if (query.select.includes(col)) {
                    fields = { ...fields, [col]: row[col] };
                }
            }
            return fields;
        });
    ...
    function matchRows(row1, row2, conditions) {
        for (const [field1, field2] of conditions) {
            if ((!!row1[field1] && row1[field1] === row2[field2])
                || (!!row1[field2] && row1[field2] === row2[field1])) {
                return true;
            }
        }
        return false;
    ...
    function join(tables, conditions) {
        while (tables.length > 1) {
            ...
            for (const row1 of table1) {
                for (const row2 of table2) {
                    if (matchRows(row1, row2, conditions)) {
    ...
    this.execute = function (queryStr) {
        const query = parseQuery(queryStr);

        const toJoin = [...query.from, ...query.join].map(name => database[name]
            .map(row => nameObjectProperly(row, name)));

        const joined = join(toJoin, query.on)
        const result = filter(joined, query);
